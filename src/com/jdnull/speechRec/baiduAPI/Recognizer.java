...
public class Recognizer {
	...
	private static final String cuid = "b8:e8:56:34:0f:b8"; // JD-Mac mac
															// address

	public static void main(String[] args) throws Exception {
		// SampleRateConverter.convert("16000", audioFromBroswer+".wav",
		// audioFromBroswer+"-formatted.wav");
		// getToken();
		// method1();
		// method2();
	...
	// FileOutputStream(audioFromBroswer+".pcm"));
	// for (String s :
	// tempStrArr) {
	// if(!s.equals("")) {
		...
		try {
			...
			while ((line = rd.readLine()) != null) {
				if (line.contains("\"result\":")) {
					...
					baiduResult = (line.substring(begin, end)).replace("]", "").replace("[", "").replace(",", "")
							.replace(" ", "").replace("，", "").replace("\"result\":", "").replace("\"", "");
					break;
				}
			}
